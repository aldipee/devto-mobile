...
function* doRequest(
  endpoint,
  {method = 'GET', language, headers, params, url = 0, formData, raw},
) {
  ...
  const allHeaders =
    currentUrl === '' ? undefined : getHeaders(language, headers, endpoint);
  let body;
...
function* retryRequest(endpoint, options) {
  try {
    while (true) {
      const {payload: isInternetReached} = yield take(
